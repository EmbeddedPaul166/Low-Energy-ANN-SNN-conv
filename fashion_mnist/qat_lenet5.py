...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-path', type=str, default='fashion_mnist/quant-relu-lenet5', required=False)
    parser.add_argument('--input-path', type=str, default='fashion_mnist/base-relu-lenet5', required=False)
    ...
    model = lenet5(10, [28, 28, 1])
    model.load_weights(input_path / 'model' / 'weights.h5')

    folded_bn_model = fold_batch_norm_layers(model, lenet5)
    quantized_model = calibrate(folded_bn_model, ds_test, 2**bits, lenet5, (-0.5, 0.5), 'cluster', 3, random_seed=seed)
