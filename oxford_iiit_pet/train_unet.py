...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-path', type=str, default='oxford_iiit_pet/base-relu-unet5', required=False)
    ...
    model = unet5(n_classes, input_shape, batch_norm=True)
    model.summary()
    ...
    ds_train, ds_test = tfds.load('oxford_iiit_pet',
                                  ...
                                  as_supervised=False,
                                  with_info=False,
                                  download=True) # Change this if necessary
    ...
    ds_val = ds_test.take(1200).map(preprocess_input).batch(batch_size, drop_remainder=False).cache().prefetch(tf.data.AUTOTUNE)
    ...
    history = model.fit(x=ds_train, validation_data=ds_val, epochs=epochs, steps_per_epoch=steps_per_epoch,
...
# Metrics results are reported at the end of qat_unet.py for pre- and post-quantization models.
