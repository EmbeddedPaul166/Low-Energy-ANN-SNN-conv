...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-path', type=str, default='cifar10/quant-relu-vgg16', required=False)
    parser.add_argument('--input-path', type=str, default='cifar10/base-relu-vgg16', required=False)
    ...
    parser.add_argument('--learning-rate', type=float, default=0.00001, required=False)
    ...
    ds_train = ds_train.cache().shuffle(1000).batch(batch_size, drop_remainder=True).map(preprocess_input).map(lambda x, y: augment(x, y, batch_size)).repeat().prefetch(tf.data.AUTOTUNE)
    ds_test = ds_test.batch(500).map(preprocess_input).cache().prefetch(tf.data.AUTOTUNE)

    model = vgg16(10, INPUT_SHAPE + [3])
    model.load_weights(input_path / 'model' / 'weights.h5')

    folded_bn_model = fold_batch_norm_layers(model, vgg16)
    quantized_model = calibrate(folded_bn_model, ds_test, 2**bits, vgg16, (-0.5, 0.5), 'cluster', 8, random_seed=seed)
    ...
    print('\nPost-quantization accuracy:', quantized_model.evaluate(ds_test)[1])
